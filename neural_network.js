...
function training(){
	var trainingText = "STEP 1: \n\nInput the number of training cycles (eg: 100,000), then click 'OK'.   NB: You may need to wait a few seconds for this to complete...";
	// Input number of training cycles
	training_num = prompt(trainingText);
	if (training_num){
		while (isNaN(training_num) || training_num>1000000 || training_num<10){
			wrong_input();
			training_num = prompt(trainingText);
		}
	} else {
	...
	training_num =  parseInt(training_num);
...
function testing(){
	var testingText = "STEP 2:\n\nNow input the number of tests (eg: 100,000) and click 'OK'.";
	test_num = prompt(testingText);
	if (test_num){
		while (isNaN(test_num) || training_num>1000000 || training_num<10){
			wrong_input();
			test_num = prompt(testingText);
		}
	} else {
	...
	if (!abort){
		correct_num = 0;
		tests = generate_inputs_list(test_num);
	...
	*/
	// 		alert(n[nodesPerLayer.length-1]);
			...
			if (output_is_correct(tests, i, nodesPerLayer)){
				correct_num += 1;
			}
		}
		success_rate = correct_num/test_num;
	}
}